...
func TestFireRed_SmokeRefClip(t *testing.T) {
	weightsDir := repoFile(t, "weights", "firered-vad")
	if _, err := os.Stat(filepath.Join(weightsDir, "model.onnx")); err != nil {
		t.Skipf("firered-vad weights not at %s: %v (run setup.sh)", weightsDir, err)
	}
	...
	m, err := vad.NewFireRed(weightsDir)
	if err != nil {
		t.Fatalf("NewFireRed: %v", err)
	}
	t.Cleanup(m.Close)
	...
	t.Logf("firered: %d segments, %.2fs total speech (%.1f%% of clip)",
