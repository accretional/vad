...
func TestMarbleNet_SmokeRefClip(t *testing.T) {
	weightsDir := repoFile(t, "weights", "marblenet")
	if _, err := os.Stat(filepath.Join(weightsDir, "model.onnx")); err != nil {
		t.Skipf("marblenet weights not at %s: %v", weightsDir, err)
	}
	...
	m, err := vad.NewMarbleNet(weightsDir)
	if err != nil {
		t.Fatalf("NewMarbleNet: %v", err)
	}
	t.Cleanup(m.Close)
	...
	t.Logf("marblenet: %d segments, %.2fs total speech (%.1f%% of clip)",
